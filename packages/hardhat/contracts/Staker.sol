...
contract Staker {
  ...
  // Balances of the user's stacked funds, think of this as a dictionary where the key is an address and the values are unsigned integers 
  ...
  // Staking deadline
  //block.timestamp gives us the current time on the blockchain, this is useful so that we can create an amount of time we want to allow for staking
  uint256 public deadline = block.timestamp + 120 seconds;

  // Contract's Events
  // A cheap form of storage that holds the transactions that occurs, will only show on the UI when emitted 
  ...
  */
  ...
  //My own functions that can check the current balance of the smart contract
